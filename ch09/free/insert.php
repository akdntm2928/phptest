 ...
 */
 ...
 if ($mode=="modify"){
     ...
     try{
        $sql = "select * from phptest.free  where num=?";  // get target record
      ...
      for ($i=0; $i<$count; $i++)						
      {
		...
		unlink($delete_path);
		   ...
		   $pdo->beginTransaction(); 
                   $sql = "update phptest.free set $field_org_name = ?, $field_real_name = ?  where num=?";
                   $stmh = $pdo->prepare($sql); 
     ...
     try{
        $pdo->beginTransaction();   
        $sql = "update phptest.free set subject=?, content=?, is_html=? where num=?";
        $stmh = $pdo->prepare($sql); 
   ...
   try{
     $pdo->beginTransaction();
     $sql = "insert into phptest.free(id, name, nick, subject, content, regist_day, hit, is_html, ";
     ...
     header("Location:http://localhost/ch09/free/list.php");
 ?>
